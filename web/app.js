...
function cacheDom() {
  ...
  refs.imageModalOpenFolderButton = document.getElementById(
    "imageModalOpenFolderButton"
  );
...
function bindEvents() {
  ...
  refs.imageModalOpenFolderButton.addEventListener("click", () => {
    if (state.imageModalRunId) {
      void openRunDirectory(state.imageModalRunId);
    }
  });
...
function renderEditHistoryList() {
  ...
  records.forEach((record) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "edit-history-item";
    ...
    button.addEventListener("click", () => {
      if (record.run_id) {
        void openRunDirectory(record.run_id);
      }
    });
    refs.editHistoryList.appendChild(button);
  });
}
...
function buildTaskCard(job) {
  ...
  card.innerHTML = `
    ...
    <div class="task-box__actions">
      ${
        record?.run_id
          ? '<button class="secondary-action secondary-action--small" type="button" data-open-run>打开目录</button>'
          : ""
      }
      ${
        record?.download_url
          ? '<button class="secondary-action secondary-action--small" type="button" data-download-run>下载 ZIP</button>'
          : ""
      }
    </div>
  `;
  ...
  const openRunButton = card.querySelector("[data-open-run]");
  if (openRunButton && record?.run_id) {
    openRunButton.addEventListener("click", () => {
      void openRunDirectory(record.run_id);
    });
  }

  const downloadButton = card.querySelector("[data-download-run]");
  if (downloadButton && record?.download_url) {
...
function appendColorMatchResults(
  container,
  { summary, runId = null, promptScrollKey = "" }
) {
...
function renderHistory() {
  ...
  visibleRecords.forEach((record) => {
    ...
    const openButton = root.querySelector(".history-open");
    const downloadButton = root.querySelector(".history-download");
    ...
    deleteButton.addEventListener("click", () => {
      void deleteRunHistory(record.run_id);
    });
...
function openImageModal(itemsOrSrc, options = {}) {
  ...
  refs.imageModalOpenFolderButton.hidden = !state.imageModalRunId;
...
function closeImageModal() {
  ...
  refs.imageModalPrevButton.disabled = true;
  refs.imageModalNextButton.disabled = true;
  refs.imageModalOpenFolderButton.hidden = true;
  state.imageModalLastPointerButton = null;
  state.imageModalContextMenuArmedUntil = 0;
...
async function deleteRunHistory(runId) {
  ...
  state.history = state.history.filter((record) => record.run_id !== runId);
  state.editConversations.forEach((conversation) => {
...
async function downloadRun(runId, relativeUrl) {
  try {
    const response = await fetch(toApiUrl(relativeUrl));
    if (!response.ok) {
    ...
    const anchor = document.createElement("a");
    anchor.href = objectUrl;
    anchor.download = `设计出图-${runId}.zip`;
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
...
async function apiFetch(url, options = {}) {
  ...
  if (!response.ok) {
    const message = isJson ? payload.error || "请求失败。" : "请求失败。";
